...
+ (NSString *) deviceModel{
    return [[self _getModel] retain];
}

+ (NSString *) deviceId{
    return [[self _getDeviceId] retain];
}

+ (NSString*) _getDeviceId{
    NSString* deviceid = [LOCALMANAGER getDeviceId];
    if (deviceid != nil){
        return deviceid;
    }else{
        [LOCALMANAGER saveDeviceId];
        deviceid = [LOCALMANAGER getDeviceId];
        return deviceid;
    }
}

+ (NSString*) _getModel{
    NSString* modelname = [LOCALMANAGER getModel];
    if (modelname != nil){
        return modelname;
    }else{
        [LOCALMANAGER saveModel];
        modelname = [LOCALMANAGER getModel];
        return modelname;
    }
}
