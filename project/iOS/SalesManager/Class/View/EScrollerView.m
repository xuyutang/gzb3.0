...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr isBig:(BOOL)bBig
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        if (bBig) {
            ...
            UIView *noteView=[[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-36,self.bounds.size.width,40)];
            [noteView setBackgroundColor:[UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:1]];
            ...
            pageControl.numberOfPages=(pageCount-2);
            [noteView addSubview:pageControl];
            noteTitle=[[UILabel alloc] initWithFrame:CGRectMake(6, 0, self.frame.size.width-pageControlWidth-15, 20)];
            [noteTitle setText:[titleArray objectAtIndex:0]];
            noteTitle.textColor = WT_WHITE;
            ...
            typeLab = [[UILabel alloc] initWithFrame:CGRectMake(6, noteTitle.bottom, 100, 20)];
            [typeLab setText:[titleArray objectAtIndex:1]];
            typeLab.textColor = WT_WHITE;
            
            [typeLab setBackgroundColor:[UIColor clearColor]];
            [typeLab setFont:[UIFont systemFontOfSize:13]];
            //[noteTitle setTextColor:WT_YELLOW];
            [noteView addSubview:typeLab];
            [self addSubview:noteView];
            [noteView release];
        }
