...
-(void)getInfoArrwithArr:(NSMutableArray*)mulArry {
    if (mulArry.count) {
        nameStr = [mulArry objectAtIndex:0];
        typeStr = [mulArry objectAtIndex:1];
    }
    _containView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bottom - 41, MAINWIDTH, 40)];
    [_containView setBackgroundColor:[UIColor colorWithRed:0.8 green:0.8 blue:0.8 alpha:0.5]];
    [self addSubview:_containView];
    UILabel *nameLab = [[UILabel alloc] initWithFrame:CGRectMake(6, 0, 200, 20)];
    nameLab.textColor = WT_WHITE;
    nameLab.text = nameStr;
    [nameLab setFont:[UIFont systemFontOfSize:13]];

    [_containView addSubview:nameLab];
    
    UILabel *typeLab = [[UILabel alloc] initWithFrame:CGRectMake(6, 20, 200, 20)];
    typeLab.textColor = WT_WHITE;
    [typeLab setFont:[UIFont systemFontOfSize:13]];

    typeLab.text = typeStr;
    [_containView addSubview:typeLab];
}
