...
- (void)viewDidLoad {
 ...
 //测试数据
  ...
  NSArray *dataArray= [NSJSONSerialization JSONObjectWithData:[datastr dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
    ...
    UIImageView *paperImageView = [[UIImageView alloc] initWithFrame:CGRectMake(60, 10, 25, 25)];
   ...
   saveImageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 10, 25, 25)];
...
-(void)toSave:(id)sender {
    ...
    //文本类型
     if ([model.type isEqualToString:@"TEXT"] ) {
         ...
         if(model.textString.trim.length > 500){
             [MESSAGE showMessageWithTitle:TITLENAME(FUNC_PAPER_POST_DES)
                               description:[NSString stringWithFormat:@"%@只能输入500个字符",model.name]type:MessageBarMessageTypeInfo
                               forDuration:INFO_MSG_DURATION];
             return;
         }
...
-(BOOL)validateNumber:(NSString *) textString {
    NSString* number=@"^-?(?=([0-9]{1,15}$|[0-9]{1,15}\\.))(0|[1-9][0-9]*)(\\.[0-9]{1,5})?$";
