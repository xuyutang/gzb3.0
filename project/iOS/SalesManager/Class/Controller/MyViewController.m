...
-(void)initUI{
    ...
    if (iPhoneX) {
        if (self.heightBool) {
            ...
            _tableView = [[PullTableView alloc]initWithFrame: CGRectMake(0, -44, MAINWIDTH, height + 44)];
        }
       
    }else {
        if (self.heightBool) {
            _tableView = [[PullTableView alloc]initWithFrame: CGRectMake(0, 0, MAINWIDTH, height - 20)];
        }else {
            _tableView = [[PullTableView alloc]initWithFrame: CGRectMake(0, -20, MAINWIDTH, height + 20)];
        }
        
    }
  //  _tableView = [[PullTableView alloc]initWithFrame: CGRectMake(0, -20, MAINWIDTH, height)];
    _tableView.delegate = self;
    _tableView.dataSource = self;
    _tableView.pullDelegate = self;
    _tableView.pullBackgroundColor = WT_YELLOW;
    self.navigationController.navigationBar.backgroundColor = WT_BLUE;
