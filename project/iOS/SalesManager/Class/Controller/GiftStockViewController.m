...
@interface GiftStockViewController ()
@end

@implementation GiftStockViewController
...
- (void) didReceiveMessage:(id)message{
    ...
    switch (INT_ACTIONTYPE(cr.type) ) {
        case ActionTypeGiftStockSave:{
            ...
            [super showMessage2:cr Title:NSLocalizedString(@"bar_gift_stock_post", @"") Description:NSLocalizedString(@"gift_stock_saved", @"")];
        }
            break;
...
-(void)toList:(id)sender{
    NSLog(@"Gift Stock ViewController to List");
    [self dismissKeyBoard];
    GiftStockListViewController *ctrl = [[GiftStockListViewController alloc] init];
...
-(void)toSave:(id)sender{
    ...
    if (imageFiles == nil || [imageFiles count]<1) {
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_stock_post", @"")
    ...
    GiftStock_Builder* sb = [GiftStock builder];
    ...
    if (DONE != [(APPDELEGATE).agent sendRequestWithType:ActionTypeGiftStockSave param:[sb build]]) {
        HUDHIDE2;
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_stock_post", @"")
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    Product_Builder* pb;
    if (index <10000) {
        pb = [((Product*)[productModelArray objectAtIndex:index]) toBuilder];
        ...
        pb = [((Product*)[productModelArray objectAtIndex:index-10000]) toBuilder];
        ...
        pb = [((Product*)[targetModelArray objectAtIndex:index-20000]) toBuilder];
        if ([textField.text length]>0) {
            [pb setNum:[textField.text doubleValue]];
        }
        ...
        pb = [((Product*)[targetModelArray objectAtIndex:index-30000]) toBuilder];
        [pb setPrice:[textField.text doubleValue]];
...
-(void)headerIsTapEvent:(id)sender{
    ...
    if (sectionIndex == 1)
    {
        bProductExpand = !bProductExpand;
    }
    if (sectionIndex == 2)
    {
        bTargetExpand = !bTargetExpand;
    }
    bEdit = NO;
...
/*
 ...
 currentPage = 0;
 productModelArray = [[LOCALMANAGER getProducts:currentCustomer.id PruductName:@"" Index:currentPage] retain];
 bHasSupply = YES;
 if ([productModelArray count]<1) {
 bHasSupply = NO;
 productModelArray = [[LOCALMANAGER getProducts:@"" Index:currentPage] retain];
 }
 bProductExpand = YES;
 bTargetExpand = YES;
 [btNumber setHidden:NO];
