...
-(void)initUI{
    ...
    tableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 45, MAINWIDTH, MAINHEIGHT - 45) style:UITableViewStylePlain];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        personCell.department.textColor = WT_WHITE;
        personCell.department.backgroundColor = WT_KHAKI;
        
         personCell.department.text = u.department.name;
        CGSize maximumLabelSize = CGSizeMake(100, 9999);
        CGSize expectSize = [personCell.department sizeThatFits:maximumLabelSize];
        //别忘了把frame给回label，如果用xib加了约束的话可以只改一个约束的值
       personCell.department.frame = CGRectMake( personCell.department.x,        personCell.name.bottom + 5, expectSize.width, expectSize.height + 5);
        personCell.distance.text = u.location.distance;
        personCell.distance.x = MAINWIDTH - personCell.distance.width -10;
        personCell.distance.y =  personCell.height - 15 -  personCell.distance.height;
        personCell.address.text = u.location.address;
        personCell.time.text = u.location.createTime;
        personCell.time.x = MAINWIDTH - personCell.time.width - 10;
