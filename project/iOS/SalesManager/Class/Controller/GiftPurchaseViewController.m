...
@interface GiftPurchaseViewController ()

@end

@implementation GiftPurchaseViewController
...
- (void)viewDidLoad
{
    ...
    [self initData];
    [self initView];
    UILabel *listImageView = [[UILabel alloc] initWithFrame:CGRectMake(60, 10, 30, 30)];
...
- (void) didReceiveMessage:(id)message{
    ...
    switch (INT_ACTIONTYPE(cr.type)) {
        case ActionTypeGiftPurchaseSave:{
            ...
            [super showMessage2:cr Title:NSLocalizedString(@"bar_gift_purchase_post", @"") Description:NSLocalizedString(@"gift_purchase_saved", @"")];
        }
            break;
...
-(void)toList:(id)sender{
    ...
    GiftPurchaseListViewController *ctrl = [[GiftPurchaseListViewController alloc] init];
...
-(void)toSave:(id)sender{
    ...
    if (imageFiles == nil || [imageFiles count]<1) {
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_purchase_post", @"")
                          description:NSLocalizedString(@"patrol_hint_files", @"")
                                 type:MessageBarMessageTypeInfo
                          forDuration:INFO_MSG_DURATION];
        return;
    }
    NSString *commentNone = @"";
    ...
    GiftPurchase_Builder* sb = [GiftPurchase builder];
    ...
    if (DONE != [(APPDELEGATE).agent sendRequestWithType:ActionTypeGiftPurchaseSave param:[sb build]]) {
        HUDHIDE2;
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_purchase_post", @"")
...
-(void)initTablesView{
    tableView = [[SMTableView alloc] initWithFrame:CGRectMake(0, 0, MAINWIDTH, MAINHEIGHT-44) style:UITableViewStylePlain];
...
//       // [((GiftProductModel *)[productModelArray objectAtIndex:i]) ] [NSNumber numberWithInt:0];
//    }
    [applyItemArray removeAllObjects];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    switch (section) {
        case 3:{
            ...
            if (!bExpand || [applyItemArray count] == 0) {
                return 0;
            }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0){
        if(indexPath.row == 1)return 120.0;
        else return 50;
    }
...
- (UIView *) tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    switch (section) {
        case 3:{
            ...
            if (bExpand) {
                header.epanfImage.image = [UIImage imageNamed:@"expander_ic_minimized.png"];
            }else {
                header.epanfImage.image = [UIImage imageNamed:@"expander_ic_maximized.png"];
            }
        ...
        case 1:{
           ...
           // _expandIcon.frame = CGRectMake(0, 0, 20, 20);
            
            if (bProductExpand) {
            header2.expandIcon.image = [UIImage imageNamed:@"expander_ic_minimized.png"];
            }else if(!bProductExpand) {
                header2.expandIcon.image = [UIImage imageNamed:@"expander_ic_maximized.png"];
            }
            [header2 addSubview:_expandIcon];
        ...
        case 2:{
            ...
            if (bTargetExpand) {
                header3.expandImage.image = [UIImage imageNamed:@"expander_ic_minimized.png"];
            }else  {
                header3.expandImage.image = [UIImage imageNamed:@"expander_ic_maximized.png"];
            }
        ...
        case 4:{
            ...
            header4.title.frame = CGRectMake(10, 5, 200, 30);
...
-(void)headerIsTapEvent:(id)sender{
    ...
    if (sectionIndex == 1) bProductExpand = !bProductExpand;
    if (sectionIndex == 2) bTargetExpand = !bTargetExpand;
    if (sectionIndex == 3)  bExpand = !bExpand;
