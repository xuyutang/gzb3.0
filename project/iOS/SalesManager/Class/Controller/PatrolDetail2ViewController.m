...
- (void)viewDidLoad {
    ...
    UIImageView *saveImageView = [[UIImageView alloc] initWithFrame:CGRectMake(100, 10, 25, 25)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            if ([_patrol.mediaType isEqualToString:PATROL_FUNC_PICTURE]) {
                ...
                for (int i = 0; i<[_patrol.filePath count]; i++) {
                    ...
                    [titles addObject:_patrol.customer
                     .category.name];
                }
            ...
            [_playVideoCell getInfoArrwithArr:titles];
            ...
            if ([_patrol.mediaType isEqualToString:PATROL_FUNC_VIDEO]) {
                UILabel *Durlab = [[UILabel alloc] initWithFrame:CGRectMake(8, cell.address.bottom, 130, 20)];
                Durlab.font = [UIFont systemFontOfSize:13];
                Durlab.textColor = WT_GRAY;
               Durlab.text = [NSString stringWithFormat:NSLocalizedString(@"video_lable_video_duration", nil), [_patrol.videoDurations objectAtIndex:0]];
                [cell addSubview:Durlab];
                
                UILabel *Sizelab = [[UILabel alloc] initWithFrame:CGRectMake(MAINWIDTH -135, cell.address.bottom, 130, 20)];
                Sizelab.font = [UIFont systemFontOfSize:13];
                Sizelab.textColor = WT_GRAY;
                Sizelab.textAlignment = 2;
               Sizelab.text =  [NSString stringWithFormat:NSLocalizedString(@"video_lable_video_size", nil),[_patrol.videoSizes objectAtIndex:0]];
        ...
        textViewCell.textView.x = 8;
        textViewCell.textView.width = MAINWIDTH - 18;
