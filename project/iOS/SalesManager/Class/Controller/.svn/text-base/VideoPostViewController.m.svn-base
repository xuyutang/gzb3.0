...
- (void)viewDidLoad {
    ...
    UILabel* listBtn = [[UILabel alloc] initWithFrame:CGRectMake(100, 10, 25, 25)];
    UIImageView *listImageView = [[UIImageView alloc] init];
    listImageView.frame = CGRectMake(100, 10, 25, 25);
    ...
    uploadImageView.frame = CGRectMake(60, 10, 25, 25);
    UIImage *uploadImage = [UIImage imageNamed:@"ab_icon_preupload.png"];
    uploadImageView.image = uploadImage;
    UILabel* uploadListBtn = [[UILabel alloc] initWithFrame:CGRectMake(60, 10, 25, 25)];
    uploadListBtn.tag = 1;
...
-(void) createButton{
    ...
    bottomBtns.buttonSelected = ^(NSInteger index){
        NSLog(@"buttonIndex:%ld",(long)index);
        if (index == 0) {
            [self btnToSaveCache];
        }else{
            [self btnToSave];
