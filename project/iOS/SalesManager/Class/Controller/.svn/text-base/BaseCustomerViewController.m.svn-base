...
//    filterBar.delegate = self;
   // [self.view addSubview:filterBar];
    ...
    if (iPhoneX) {
         tagSelectVC.view.frame = CGRectMake(0, 90, MAINWIDTH, SCREENHEIGHT - 216 - 64);
    }else{
    ...
    [tagSelectVC release];
    if (iPhoneX) {
          tableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 90, MAINWIDTH, SCREENHEIGHT - STATEBARHEIGHT - 64  -140) style:UITableViewStylePlain];
    }else {
       tableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 90, MAINWIDTH, SCREENHEIGHT - STATEBARHEIGHT -140) style:UITableViewStylePlain];
    ...
    if (indexPath.section == 0) {
        ...
        if (c.id == -1) {
            ...
            if (c.location.latitude > 0) {
                ...
                if (!_bHideFuncBtn) {
                    customerCell.btLocate.width = MAINWIDTH - customerCell.btLocate.width - 5;
                    CGRect rectl = customerCell.locate.frame;
                    UILabel *lLocate = [[UILabel alloc] initWithFrame:CGRectMake(MAINWIDTH - 26, rectl.origin.y + 5, rectl.size.width, rectl.size.height)];
            ...
            customerCell.address.width = MAINWIDTH - 35;
            customerCell.address.text = [NSString stringWithFormat:@"%@",c.location.address];
            
            customerCell.btFunction.x = MAINWIDTH - 5 - customerCell.btFunction.width;
            if (!_bHideFuncBtn) {
                CGRect rect = customerCell.cloud.frame;
                UILabel *lClound = [[UILabel alloc] initWithFrame:CGRectMake(MAINWIDTH - 26, rect.origin.y + 5, rect.size.width, rect.size.height)];
