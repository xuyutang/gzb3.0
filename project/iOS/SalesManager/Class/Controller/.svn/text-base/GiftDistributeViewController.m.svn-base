//
//  GiftDistributeViewController.m
//  SalesManager
//
...
@interface GiftDistributeViewController ()

@end

@implementation GiftDistributeViewController
...
-(void)toList:(id)sender{
    ...
    GiftDistributeListViewController *ctrl = [[GiftDistributeListViewController alloc] init];
...
-(void)toSave:(id)sender{
    ...
    if (currentCustomer == nil) {
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_distibute_post", @"")
    ...
    GiftDistribute_Builder* sb = [GiftDistribute builder];
    ...
    if (DONE != [(APPDELEGATE).agent sendRequestWithType:ActionTypeGiftDistributeSave param:[sb build]]) {
        HUDHIDE2;
        [MESSAGE showMessageWithTitle:NSLocalizedString(@"bar_gift_distibute_post", @"")
...
- (void) didReceiveMessage:(id)message{
    ...
    switch (INT_ACTIONTYPE(cr.type)) {
        case ActionTypeGiftDistributeSave:{
            ...
            [super showMessage2:cr Title:NSLocalizedString(@"bar_gift_distibute_post", @"") Description:NSLocalizedString(@"gift_distribute_saved", @"")];
        }
            break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        ...
        case 0:{
            ...
            [self.parentCtrl presentViewController:customerNavCtrl animated:YES completion:nil];
            [ctrl release];
        }
            break;
        case 2:{
