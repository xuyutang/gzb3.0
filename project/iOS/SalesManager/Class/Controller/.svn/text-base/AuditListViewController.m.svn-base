...
@interface AuditListViewController ()<RefreshDelegate,PullTableViewDelegate,HeaderSearchBarDelegate,NameFilterViewControllerDelegate,DepartmentViewControllerDelegate,DropMenuDelegate>
...
@synthesize auditArray,bEnableSelect;
...
- (void)viewDidLoad {
    ...
    NSArray* titles2 = [NSArray arrayWithObjects:@"待处理",@"已处理", nil];
    ...
    //数据源
    if (iPhoneX) {
        self.pullTableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 90, MAINWIDTH, SCREENHEIGHT -216 ) style:UITableViewStylePlain];
    }else {
        self.pullTableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 90, MAINWIDTH, MAINHEIGHT - 90) style:UITableViewStylePlain];
    }
    ...
    if (iPhoneX) {
        tmpFrame.size.height = SCREENHEIGHT -216;
    }else {
    ...
    if (bEnableSelect) {
        ...
        UILabel *saveImageView = [[UILabel alloc] initWithFrame:CGRectMake(100, 10, 30, 30)];
    ...
    currentPage = 1;
    auditArray = [[NSMutableArray alloc] init];
    selectedArray = [[NSMutableArray alloc] init];
    [lblFunctionName setText:[NSString stringWithFormat:@"%@-审核",TITLENAME(FUNC_APPROVE_DES)]];
...
//        [pb setPage:1];
//        [pb setAuditStatus:[NSString stringWithFormat:@"%ld",(long)APPLY_NOT_MANAGE]];
//        applyParams = [[pb build] retain];
//        
//        AGENT.delegate = self;
//        if (DONE != [AGENT sendRequestWithType:ActionTypeApplyItemAuditList param:applyParams]){
...
-(void) refreshParams {
    ...
    [aipb setAuditStatus:statusString];
...
-(void)HeaderSearchBarClickBtn:(int)index current:(int)current andWithTag:(int)tag{
    ...
    if (tag == 9999) {
        ...
        ApplyItemParams_Builder* pb = [applyParams toBuilder];
        if (index == 0) {
           statusString = [NSString stringWithFormat:@"%ld",(long)APPLY_NOT_MANAGE];
           
        }else {
             statusString = [NSString stringWithFormat:@"%ld",(long)APPLY_MANAGE];
            
        
        }
        [pb setAuditStatus:statusString];
        applyParams = [[pb build] retain];
        AGENT.delegate = self;
        if (DONE != [AGENT sendRequestWithType:ActionTypeApplyItemAuditList param:applyParams]){
            [MESSAGE showMessageWithTitle:TITLENAME(FUNC_APPROVE_DES)
...
- (void) didReceiveMessage:(id)message {
    ...
    if ((INT_ACTIONTYPE(cr.type) == ActionTypeApplyItemAuditList) && ([cr.code isEqual: NS_ACTIONCODE(ActionCodeDone)])){
    ...
    if ((INT_ACTIONTYPE(cr.type) == ActionTypeApplyItemReply) && ([cr.code isEqual: NS_ACTIONCODE(ActionCodeDone)])) {
        ...
        if (auditArray != nil){
            ...
            [auditArray removeObjectAtIndex:currentRow];
            [auditArray insertObject:[wb build] atIndex:currentRow];
    ...
    if ((INT_ACTIONTYPE(cr.type) == ActionTypeApplyItemAuditList) || (INT_ACTIONCODE(cr.code) != ActionCodeDone)){
        [self.pullTableView reloadData];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ApplyItem *w = [auditArray objectAtIndex:indexPath.row];
    ...
    cell.content.width = MAINWIDTH - cell.content.x - 15;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [self _showApplyItemDetail:[auditArray objectAtIndex:indexPath.row]];
}
...
- (void)didSelectedCheckBox:(QCheckBox *)checkbox checked:(BOOL)checked {
    if (checked) {
        [selectedArray addObject:[auditArray objectAtIndex:checkbox.tag]];
    }else{
        [selectedArray removeObject:[auditArray objectAtIndex:checkbox.tag]];
    }
}
...
- (void) didFinishInput:(int)tag Input:(NSString *)input {
    int applyItemId = ((ApplyItem*)[auditArray objectAtIndex:tag]).id;
    ...
    currentRow = tag;
    ApplyItem* w = [[auditArray objectAtIndex:tag] retain];
...
- (void) refresh:(id)obj {
    if ([obj isKindOfClass:[ApplyItem class]]) {
        for (int i = 0; i < auditArray.count; i++) {
            ...
            if (((ApplyItem*)[auditArray objectAtIndex:i]).id == applyItem.id){
                [auditArray removeObjectAtIndex:i];
                [auditArray insertObject:applyItem atIndex:i];
